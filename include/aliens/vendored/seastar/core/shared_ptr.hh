// SI Aug 2016 -- mostly copied from Seastar.
// only changes are for compatibility with C++11
// (seastar requires c++14)
...
namespace aliens { namespace vendored { namespace seastar {
...
using shared_ptr_impl
    = typename std::conditional<
        std::is_base_of<enable_lw_shared_from_this<typename std::remove_const<T>::type>, T>::value,
        enable_lw_shared_from_this<typename std::remove_const<T>::type>,
        shared_ptr_no_esft<typename std::remove_const<T>::type>
      >::type;
...
public:
    ...
    bool operator==(const lw_shared_ptr<typename std::remove_const<T>::type>& x) const {
        return _p == x._p;
    }
...
private:
    ...
    explicit shared_ptr(enable_shared_from_this<typename std::remove_const<T>::type>* p) noexcept : _b(p), _p(static_cast<T*>(p)) {
        if (_b) {
            ++_b->count;
        }
    }
...
enable_shared_from_this<T>::shared_from_this() {
    auto unconst = reinterpret_cast<enable_shared_from_this<typename std::remove_const<T>::type>*>(this);
...
enable_shared_from_this<T>::shared_from_this() const {
    ...
    auto unconst = reinterpret_cast<enable_shared_from_this<typename std::remove_const<T>::type>*>(esft);
...
}}} // aliens::vendored::seastar


namespace std {

template <typename T>
struct hash<aliens::vendored::seastar::lw_shared_ptr<T>> : private hash<T*> {
    size_t operator()(const aliens::vendored::seastar::lw_shared_ptr<T>& p) const {
        return hash<T*>::operator()(p.get());
    }
};

template <typename T>
struct hash<aliens::vendored::seastar::shared_ptr<T>> : private hash<T*> {
    size_t operator()(const aliens::vendored::seastar::shared_ptr<T>& p) const {
        return hash<T*>::operator()(p.get());
    }
};
