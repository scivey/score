...
#include "evs/events2/wheel/DynamicWheelBuffer.h"
...
class HTimerWheel3 {
  ...
  class Bucket {
   ...
   public:
    ...
    void say(const char *fmt, Types&&... args) {
      ...
      EVS_INFO("Bucket[{}/{}]: '{}'", getResolution(), getNumSlots(), subMessage);
    }
 ...
 protected:
  ...
  HTimerWheel3(base_t *base, const WheelSettings& settings)
    : base_(base), wheelSettings_(settings) {}

  EVS_DISABLE_COPY_AND_ASSIGN(HTimerWheel3);
  ...
  void addEntry(Entry&& entry) {
    ...
    for (size_t i = 0; i < levels_.size(); i++) {
      ...
      if (offset < level.bucket.timeCapacity()) {
        // EVS_INFO("entry has offset {}; adding to bucket{}.", offset.count(), i+1);
 ...
 public:
  static HTimerWheel3* createNew(base_t *base, const WheelSettings& settings) {
    auto wheel = new HTimerWheel3(base, settings);
    wheel->initLevels();
    return wheel;
  }
