...
#include "score/vendored/folly/Malloc.h"
#include "score/vendored/folly/Traits.h"
...
#define SCORE_FOLLY_FBV_UNROLL_PTR(first, last, OP) do {  \
...
private:
  ...
  static void S_destroy_range(T* first, T* last) noexcept {
    if (!boost::has_trivial_destructor<T>::value) {
      ...
      #define SCORE_FOLLY_FBV_OP(p) (p)->~T()
      SCORE_FOLLY_FBV_UNROLL_PTR(first, last, SCORE_FOLLY_FBV_OP)
      #undef SCORE_FOLLY_FBV_OP
    }
  }
...
private: // we have the private section first because it defines some macros
  ...
  //  some macros: SCORE_FOLLY_FBVECTOR_INSERT_(PRE|START|TRY|END)
  ...
  #define SCORE_FOLLY_FBVECTOR_INSERT_PRE(cpos, n)                                  \
  ...
  #define SCORE_FOLLY_FBVECTOR_INSERT_START(cpos, n)                                \
      }                                                                       \
      assert(isValid(cpos));                                                  \
    }                                                                         \
  ...
  #define SCORE_FOLLY_FBVECTOR_INSERT_TRY(cpos, n)                                  \
    } catch (...) {                                                           \
      if (fresh) {                                                            \
  ...
  #define SCORE_FOLLY_FBVECTOR_INSERT_END(cpos, n)                                  \
        M_deallocate(b, newCap);                                              \
        throw;                                                                \
      }                                                                       \
...
public:
  ...
  iterator emplace(const_iterator cpos, Args&&... args) {
    SCORE_FOLLY_FBVECTOR_INSERT_PRE(cpos, 1)
    SCORE_FOLLY_FBVECTOR_INSERT_START(cpos, 1)
      M_construct(start, std::forward<Args>(args)...);
    SCORE_FOLLY_FBVECTOR_INSERT_TRY(cpos, 1)
      M_destroy(start);
    SCORE_FOLLY_FBVECTOR_INSERT_END(cpos, 1)
  }
  ...
  iterator insert(const_iterator cpos, size_type n, VT value) {
    SCORE_FOLLY_FBVECTOR_INSERT_PRE(cpos, n)
      if (dataIsInternalAndNotVT(value)) return insert(cpos, n, T(value));
    SCORE_FOLLY_FBVECTOR_INSERT_START(cpos, n)
      D_uninitialized_fill_n_a(start, n, value);
    SCORE_FOLLY_FBVECTOR_INSERT_TRY(cpos, n)
      D_destroy_range_a(start, start + n);
    SCORE_FOLLY_FBVECTOR_INSERT_END(cpos, n)
  }
