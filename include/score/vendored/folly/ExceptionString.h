// SI
// adapted from folly's ExceptionString.h to remove
// dependency on fbstring
...
#include "score/vendored/folly/Portability.h"
 ...
 * defined.
 */
inline sf_string_t exceptionStr(const std::exception& e) {
#ifdef SCORE_FOLLY_HAS_RTTI
  sf_string_t rv(demangle(typeid(e)));
  rv += ": ";
#else
  sf_string_t rv("Exception (no RTTI available): ");
#endif
  rv += e.what();
  return rv;
}
...
inline sf_string_t exceptionStr(std::exception_ptr ep) {
  try {
...
auto exceptionStr(const E& e) -> typename std::
    enable_if<!std::is_base_of<std::exception, E>::value, sf_string_t>::type {
#ifdef SCORE_FOLLY_HAS_RTTI
...
}}} // namespace score::vendored::folly
