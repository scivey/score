...
#if SCORE_FOLLY_HAVE_CPLUS_DEMANGLE_V3_CALLBACK
# include <cxxabi.h>
...
std::string demangle(const char* name) {
#ifdef SCORE_FOLLY_DEMANGLE_MAX_SYMBOL_SIZE
  ...
  if (mangledLen > SCORE_FOLLY_DEMANGLE_MAX_SYMBOL_SIZE) {
    return std::string(name, mangledLen);
  }
#endif
