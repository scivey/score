...
namespace score { namespace vendored { namespace seastar {
...
}}} // score::vendored::seastar


namespace std {

template <typename T>
struct hash<score::vendored::seastar::lw_shared_ptr<T>> : private hash<T*> {
    size_t operator()(const score::vendored::seastar::lw_shared_ptr<T>& p) const {
        return hash<T*>::operator()(p.get());
    }
};

template <typename T>
struct hash<score::vendored::seastar::shared_ptr<T>> : private hash<T*> {
    size_t operator()(const score::vendored::seastar::shared_ptr<T>& p) const {
        return hash<T*>::operator()(p.get());
    }
};
