...
#include "score/vendored/folly/detail/ExceptionWrapper.h"
...
protected:
  ...
  void assign_eptr(std::exception_ptr eptr, Ex& e) {
    this->eptr_ = eptr;
    this->estr_ = std::string{ exceptionStr(e) };
    this->ename_ = std::string{ demangle(typeid(e)) };
  }
...
inline sf_string_t exceptionStr(const exception_wrapper& ew) {
  return ew.what();
}
